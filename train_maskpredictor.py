...
from src.datasets.weights.tokenized_model_weights import TokenizedAlignedZooDataset, SparsifiedZooDataset
...
def main(cfg):
    ...
    pruners = ["chita", "random", "magnitude", "l_obs", "woodfisher"]
    ...
    if cfg.experiment.mode == "base":
        mode = mode + "_base"
        train_set = SparsifiedZooDataset(all_orig, all_spars, tokenizer, cfg.transformer.blocksize, stride=stride, split_indices=train_indices)
        ...
        train_set = SparsifiedZooDataset(all_orig, all_spars, tokenizer, cfg.transformer.blocksize, stride=stride, split_indices=train_indices, noise_percentage=cfg.experiment.noise_percentage)
    print("Loading validation models...")
    val_set = SparsifiedZooDataset(all_orig, all_spars, tokenizer, cfg.transformer.blocksize, stride=stride, split_indices=val_indices)
    print("Loading testing models...")
    test_set = TokenizedAlignedZooDataset(all_orig, tokenizer, cfg.transformer.blocksize, stride=stride, split_indices=test_indices)
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="maskpredictor-{epoch:02d}-{val_loss:.4f}",
        monitor="val_loss",
        mode="min",
        save_top_k=1
    )
