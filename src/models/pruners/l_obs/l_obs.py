...
def prune_weights(store_location, checkpoint, dataset_name, data_dir, model_name, img_size):
    ...
    hessian_inverse_root = Path("src/pruners/l_obs") / current_path / "hessian_inv"
    hessian_inverse_root.mkdir(parents=True, exist_ok=True)
    hessian_root = Path("src/pruners/l_obs") / current_path / "hessian"
    ...
    for layer_name in layer_name_list:
        if layer_name in ['module_list.0', 'module_list.3', 'module_list.6']:
            ...
            np.save(hessian_root / f"{layer_name}.npy", hessian)
            # Inverse Hessian
            try:
        ...
        np.save(hessian_inverse_root / f"{layer_name}.npy", hessian_inv)
    ...
    for sparsity in target_sparsities:
        ...
        for layer_name in layer_name_list:
            if layer_name in ['module_list.0', 'module_list.3', 'module_list.6']:
                layer_type = 'C'
                kernel = checkpoint[f'{layer_name}.weight'].data.numpy()
                kernel_shape = kernel.shape
                weight = unfold_kernel(kernel)
                bias = checkpoint[f'{layer_name}.bias'].data.numpy()
                wb = np.concatenate([weight, bias.reshape(1, -1)], axis = 0)
            elif layer_name in ['module_list.9', 'module_list.11']:
                layer_type = 'F'
                weight = checkpoint[f'{layer_name}.weight'].data.numpy()
                bias = checkpoint[f'{layer_name}.bias'].data.numpy()
